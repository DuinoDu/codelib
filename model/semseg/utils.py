...
class conv2DBatchNormRelu(nn.Module):
    def __init__(self, in_channels, n_filters, k_size,  stride, padding, bias=True):
        ...
        self.cbr_unit = nn.Sequential(
                nn.Conv2d(int(in_channels), int(n_filters), kernel_size=k_size,
                          padding=padding, stride=stride, bias=bias),
                nn.BatchNorm2d(int(n_filters)),
    ...
    def __init__(self, in_channels, n_filters, k_size, stride, padding, bias=True):
        ...
        self.dcbr_unit = nn.Sequential(
                nn.ConvTranspose2d(int(in_channels), int(n_filters), kernel_size=k_size,
                    padding=padding, output_padding=padding, stride=stride, bias=bias),
                nn.BatchNorm2d(int(n_filters)),
                nn.ReLU(inplace=True),)
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv):
        ...
        if is_deconv:
            ...
            self.up = nn.Upsampling(scale_factor=2, mode='bilinear')
...
class linknetUp(nn.Module):
    def __init__(self, in_channels, n_filters):
        ...
        # B, C/2, H, W -> B, C/2, H*2, W*2
        self.deconvbnrelu2 = deconv2DBatchNormRelu(n_filters/2, n_filters/2, k_size=3,  stride=2, padding=1)
