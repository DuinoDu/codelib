...
def show_centers(img, centers):
    imshow(img)
    
    pts = torch.Tensor([x for key in centers.keys() for x in centers[key]])
    for i in range(pts.size(0)):
        print(pts[i])
        plt.plot(pts[i, 0], pts[i, 1], 'yo')
    plt.axis('off')
...
def draw_heatmap_on_image(input, heatmap, size=512):
    input = custom.unnormalize(input) 
    input = resize(input, size, size)
    
    heatmap.transpose_(0, 1)
    heatmap = resize(color_heatmap(heatmap), size, size)
    out = input*0.5 + heatmap*0.5
    return out

def show_image_with_heatmap(input, heatmap, size=512):
    out = draw_heatmap_on_image(input, heatmap, size)
    imshow(out)
    plt.show()
